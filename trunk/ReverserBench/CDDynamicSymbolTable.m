...
-(void)_process;
{
	...
	// Indirect Symbols;
	indirectSymbols = [[NSMutableArray alloc] init];
	int endIndirectSymbols = dysymtab.indirectsymoff + (dysymtab.nindirectsyms * sizeof(unsigned long));
	
	for (i = dysymtab.indirectsymoff; i < endIndirectSymbols; i += sizeof(unsigned long))
	{
		unsigned long index = *(unsigned long*)[nonretainedMachOFile bytesAtOffset:i];
		[indirectSymbols addObject:[NSNumber numberWithUnsignedLong:index]];
	}
}
